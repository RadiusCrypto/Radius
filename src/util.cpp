...
const char * const BITCOIN_CONF_FILENAME = "radius.conf";
const char * const BITCOIN_PID_FILENAME = "radiusd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "radius" is a composite category enabling all Radius-related debug output
            if(ptrCategory->count(string("radius"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RadiusCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RadiusCore
    // Mac: ~/Library/Application Support/RadiusCore
    // Unix: ~/.radiuscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RadiusCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RadiusCore";
#else
    // Unix
    return pathRet / ".radiuscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty radius.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override radius.conf
